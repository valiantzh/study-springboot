...
import com.study.upms.constant.Codes;
...
public class AuthController {
    ...
    public Json login(@RequestBody String body){
        ...
        try {
            ...
            //从session取出用户信息
            SysUserBO userBO = (SysUserBO) currentUser.getPrincipal();
            if (userBO == null) throw new AuthenticationException();

            log.info("user login: {}, sessionId: {}", userBO.getUser().getUname(),currentUser.getSession().getId());
            //返回登录用户的信息给前台，含用户的所有角色和权限
            return Json.succ(oper)
                    .data("token", UUID.randomUUID().toString())
                    .data("uid", userBO.getUser().getUid())
                    .data("nick", userBO.getUser().getNick())
                    .data("roles", userBO.getRoles())
                    .data("perms", userBO.getPerms());
    ...
    public Json info(){
        ...
        //从session取出用户信息
        SysUserBO userBO = (SysUserBO) subject.getPrincipal();
        if (userBO==null){
            //告知前台，登录失效
            ...
            //返回登录用户的信息给前台，含用户的所有角色和权限
            return Json.succ(oper)
                    .data("name",userBO.getUser().getUname())
                    .data("nick",userBO.getUser().getNick())
                    .data("avator","")
                    .data("roles",userBO.getRoles())
                    .data("perms",userBO.getPerms());
