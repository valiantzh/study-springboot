...
import static com.study.common.util.StringUtil.lineToHump;
...
public class MybatisGeneratorUtil {
	 ...
	 * @param module        项目模块
	 * @param subModulePrefix    子模块前缀
	 * @param database      数据库
	 * @param tablePrefix  表前缀
	 * @param packageName  包名
	 * @param genServiceFlag  是否生成Service模板
	 * @param genMockFlag 是否生成Mock模板
	 */
	public static void generator(
			...
			String jdbcPassword,
			String module,
			String subModulePrefix,
			String database,
			String tablePrefix,
			String packageName,
			boolean genServiceFlag,
			boolean genMockFlag,
			Map<String, String> lastInsertIdTables) throws Exception{

		boolean isSplitSubModule = false;//是否拆分子模块
		if(StringUtil.isNotBlank(subModulePrefix)){//子模块前缀不为空，表示需要拆分dao,service，server模块
			isSplitSubModule = true;
		}
		String os = System.getProperty("os.name");
		String modulePath = module;
		if(isSplitSubModule){
			modulePath = modulePath + "/" + subModulePrefix;
		}

		String targetProject = modulePath ;
		if(isSplitSubModule){
			targetProject = targetProject + "-dao";
		}
		...
		try {
			...
			String targetProjectSqlMap = basePath + modulePath;
			if(isSplitSubModule){
				targetProjectSqlMap = targetProjectSqlMap+ "-dao";//service
			}
			context.put("tables", tables);
			context.put("generator_javaModelGenerator_targetPackage", packageName + ".dao.model");
			context.put("generator_sqlMapGenerator_targetPackage", packageName + ".dao.mbg.mapper");
			context.put("generator_javaClientGenerator_targetPackage", packageName + ".dao.mbg.mapper");
			...
			deleteDir(new File(targetProject + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/dao/mbg/mapper"));
			deleteDir(new File(targetProjectSqlMap + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/dao/mbg/mapper"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		String serviceBasePath = basePath + modulePath;
		if(isSplitSubModule){
			serviceBasePath = serviceBasePath+ "-service";
		}
		String servicePath = serviceBasePath  + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/service/api";
		String serviceMockPath = serviceBasePath + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/service/mock";
		String serviceImplPath = serviceBasePath + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/service/impl";
